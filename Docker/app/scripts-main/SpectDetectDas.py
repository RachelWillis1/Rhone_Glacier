...
from pydvs.readers import stream_comparison as compare
from pydvs.readers import peak_h5_native_data, peak_h5_asdf_data, peak_h5_idas2_data
...
def get_multiple_hdf5_file_data(native_files, channels=[0,-1]):
    '''Function to get hdf5 data for a hdf5 file or a set of multiple consectuive hdf5 files.
    Returns st - stream containing detrended mseed data.'''
    st_array = reader(native_files, stream=False, channels=channels, h5type='idas2', debug=True)
    st_array[0] = st_array[0] - st_array[0].mean(axis=1).reshape(-1, 1) # The mean of the data is subtracted from the 2D array
    return st_array
...
def whitened_spectrogram(st_array, header, statNumbers, matplotlib_vs_obspy_plotting_switch, verbosity_level=0):
    ...
    # Process data:
    nt, nx = st_array.shape

    fs = st_array[2]["sampling_frequency_Hz"]
    data = st_array[0][:,statNumbers[0]:statNumbers[1]]
    NFFT = int(fs)*0.126 # Number of values used in each block (use int(fs)*0.05 for 90 sec data and int(fs)*1 for hour long data)

    Pxx = [mlab.psd(channel, Fs=fs, NFFT=NFFT) for channel in data.T]
    freqs = Pxx[0][1]
    Pxx = np.array(Pxx)[:,0].sum(axis=0) #calculate the amplitude spectral density
    # plt.figure()
    ...
    freqs = np.fft.rfftfreq(nt, 1/fs) # get the discrete fourier transform sample frequencies
    f_data = sum([np.fft.rfft(channel) for channel in data.T]) #transform data to freqency domain
...
def icequake_search_using_spectrogram(st, num_stacks, freq_range, min_snr, verbosity_level=0):
    """Function to search for icequakes using spectrogram. Won't pick up all events but will pick up
    a number for cross-correlation. Input is a stream, and a list of station names that are in the
    stream to be processed.
    Returns detection_all_stations_bins, an array of length number of seconds of data input (3600).
    This array contains th fraction of total stations that detected an event in the spectrogram
    for a given second. This can then be used to search for an exact event origin time."""

    # Create arrays for storing data:
    num_seconds_of_data = int((st[0].shape[0])/st[2]['sampling_frequency_Hz']) #3600 if 1h data
    num_channels = st[0].shape[1]
    # array to store detections at stations. A value of 1 is appended to a
    # specific bin if an event is detected there, above the snr threshold specified below
    detection_all_stations_bins = np.zeros(num_seconds_of_data, dtype=float)

    # Loop over all stations, getting spectrum and appending to total spectrum array:
    for i in range(0, num_channels, num_stacks):
        try:
            data = st[0][:,i:i+10].sum(axis=1) # Linearly stack 10 consecutive num_channels
        except:
            print("Last channels to process: Can't stack 10 channels."
            data = st[0][:,i:].sum(axis=1)

        # statName = statName_list[i]
        if verbosity_level==2:
            print("Processing data for channels: {}-{}".format(i, i+10)

        # Get amplitude spectral density (ASD) (unwhitened) (normalised to 1):
        # Process data:
        data = data / np.max(data)

        freqs, Sxx = scipy.signal.welch(data, fs=fs, nperseg=nperseg) #calculate the amplitude spectral density

        # Perform spectral whitening:
        psd = scipy.interpolate.interp1d(freqs, Sxx) # interpolates the ASD (unwhitened)
        ...
        white_f_data = f_data / (np.sqrt(PSDs * fs /2.))
        white_data = np.fft.irfft(white_f_data, n=len(data))

        # Get spectrogram data
        f, t, Pxx = scipy.signal.spectrogram(white_data, fs=fs, nperseg=nperseg)
        ...
        #find peaks within a window of size 1 with SNR>2.3 # (widths=[1] doesn't work due to scipy version on uranus!, so pass it a numpy array)
        peak_idx = scipy.signal.find_peaks_cwt(freq_av_power, np.array([1]), min_snr=min_snr)
