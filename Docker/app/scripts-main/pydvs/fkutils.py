...
def create_mask_BP(fk_data, freq_axis, k_axis, k_slope=10,
                   corner_freqs=[1, 100, -0.1, 0.1]):
    """
    Function to create filter mask for 2D bandpass filter
    :type corner_freqs: list
    :param corner_freqs: List giving corner frequencies for 2D bandpass filter
    as [fmin, fmax, kmin, kmax]
    """
...
#     k_slope = k_max/6
    ...
    # Corner are chosen to contain the lower values coresponding to
    # basic roll-off right upper corner
...
# '''
# vel_mask - use the results of vel_map to create a filter that band passes
#            phase velocities
#          - between vel_low and vel_high. The smoothing is applied so that
#            the edges are not too
...
def vel_mask(velmap, vel_low, vel_high, smooth_perc=[-0.1, 0.1]):
    ...
    -------
    mask : numpy.ndarray
        The filter mask in the FK domain. Note that the mask only contains
...
def AGC(data_in, window, normalize=False, time_axis='vertical'):
    ...
    time_axis : string, optional
        ...
        horizontal axis. Obspy Streams use horizontal axis by default. The
        default is 'vertical'.
    ...
    """
    # data: input data as numpy array, time is on the vertical axis, channel
