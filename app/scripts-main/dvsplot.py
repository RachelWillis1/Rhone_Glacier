...
def plotChRange(data, startCh, endCh, samplesPerSec=1000, clipPerc=95,
    ...
    plt.figure(dpi=dpi)
    plt.imshow(data, aspect='auto', interpolation='none', vmin=-clip,
               vmax=clip, cmap=cmap, extent=(0, nSec, endCh-1, startCh))
    plt.xlabel('Time (s)')
    plt.ylabel('Channel number')
    # plt.title('{} - {}'.format(st0[0].stats.starttime + 3.6,
    #                            st0[0].stats.starttime + 4), fontsize=8)
    ...
    if outfile:
        plt.savefig(outfile)
        # plt.savefig('report/3000deepQuake-ch-'+str(startCh)+'-'
        #             + str(endCh)+'.png')
#     plt.clf()
    plt.show()
    return
