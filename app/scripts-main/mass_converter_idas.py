...
def mass_converter_idas2(
        input_directory, output_directory, check_files_every_n=5,
        check_existing=True, debug=False):
    """Mass converter, converting native silixa .h5 files from input_directory
    to converted idas .hdf5 files in output_directory

    Args:
        input_directory (pathlib.Path): path of input directory.
            Will browse all subdirectories for .h5 files
        output_directory (pathlib.Path): output directory.
            Function will create folders for each day separately
        check_files_every_n (int, optional): Frequency with which the input and
            output files are compared. If=1, then every file will compare its
            inout data to the data in the written file. This slows down
            everything significantly. Defaults to 5.
        check_existing (bool, optional): If a converted file exists based on
            the input file, then check if the file contents are the same. Note
            that this also only takes the check_files_every_n into account!.
            Defaults to True.
    """
...
if __name__ == "__main__":
    in_folder = ("/Users/patrick/polybox/test_data/idas_readers/test_data/"
                 "h5_native")
    tmp_folders = ("/Users/patrick/polybox/test_data/idas_readers/test_data/"
                   "h5_idas2/")

    mass_converter_idas2(in_folder, tmp_folders,
                         check_files_every_n=5, check_existing=True,
                         debug=False)
